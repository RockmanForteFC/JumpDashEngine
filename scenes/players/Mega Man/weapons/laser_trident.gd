...
#      Constants
#-------------------------------------------------
const Projectile: Resource = preload("res://scenes/players/Mega Man/projectiles/Laser Trident/Laser trident.tscn")
const LASER_TRIDENT_MAX_ON_SCREEN:int = 3
#-------------------------------------------------
#      Signals
#-------------------------------------------------
...
#-------------------------------------------------
func can_use() -> bool:
	var on_screen_bullets: Array = get_tree().get_nodes_in_group("LaserTridentP%s" % owner.player_number)
	return on_screen_bullets.size() < LASER_TRIDENT_MAX_ON_SCREEN and weapon_energy > 0
...
func _get_bullet() -> Node:
	...
	bullet.add_to_group("LaserTridentP%s" % owner.player_number)
