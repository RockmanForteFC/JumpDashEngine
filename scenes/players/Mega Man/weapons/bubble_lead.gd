...
#      Constants
#-------------------------------------------------
const Projectile: Resource = preload("res://scenes/players/Mega Man/projectiles/bubble_lead/bubble_lead.tscn")
const BUBBLE_LEAD_MAX_ON_SCREEN:int = 2
#-------------------------------------------------
#      Signals
#-------------------------------------------------
...
#-------------------------------------------------
func can_use() -> bool:
	var on_screen_bullets: Array = get_tree().get_nodes_in_group("BubbleLeadP%s" % owner.player_number)
	return on_screen_bullets.size() < BUBBLE_LEAD_MAX_ON_SCREEN and weapon_energy > 0
...
func _get_bullet() -> Node:
	...
	bullet.add_to_group("BubbleLeadP%s" % owner.player_number)
