...
#      Constants
#-------------------------------------------------
const Projectile: Resource = preload("res://scenes/players/Mega Man/projectiles/flame_blast/flame_blast.tscn")
const FLAME_PILLAR_MAX_ON_SCREEN:int = 3
#-------------------------------------------------
#      Signals
#-------------------------------------------------
...
#-------------------------------------------------
func can_use() -> bool:
	var on_screen_bullets: Array = get_tree().get_nodes_in_group("FlameBlastP%s" % owner.player_number)
	return on_screen_bullets.size() < FLAME_PILLAR_MAX_ON_SCREEN and weapon_energy > 0
...
func _get_bullet() -> Node:
	...
	bullet.add_to_group("FlameBlastP%s" % owner.player_number)
