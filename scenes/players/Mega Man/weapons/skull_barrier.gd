...
#      Constants
#-------------------------------------------------
const Projectile: Resource = preload("res://scenes/players/Mega Man/projectiles/Skull_barrier/Skull_barrier.tscn")
const SKULL_BARRIER_MAX_ON_SCREEN:int = 1
#-------------------------------------------------
#      Signals
#-------------------------------------------------
...
#-------------------------------------------------
func can_use() -> bool:
	var on_screen_bullets: Array = get_tree().get_nodes_in_group("SkullBarrierP%s" % owner.player_number)
	return on_screen_bullets.size() < SKULL_BARRIER_MAX_ON_SCREEN and weapon_energy > 0
...
func _get_bullet() -> Node:
	...
	bullet.add_to_group("SkullBarrierP%s" % owner.player_number)
