...
#      Constants
#-------------------------------------------------
const Projectile: Resource = preload("res://scenes/players/Mega Man/projectiles/gemini_laser/gemini_laser_controller.tscn")
const GEMINI_LASER_MAX_ON_SCREEN:int = 3
#-------------------------------------------------
#      Signals
#-------------------------------------------------
...
#-------------------------------------------------
func can_use() -> bool:
	var on_screen_bullets: Array = get_tree().get_nodes_in_group("GeminiLaserP%s" % owner.player_number)
	return on_screen_bullets.size() < GEMINI_LASER_MAX_ON_SCREEN and weapon_energy > 0
...
func _get_bullet() -> Node:
	...
	bullet.add_to_group("GeminiLaserP%s" % owner.player_number)
